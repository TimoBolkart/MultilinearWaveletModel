...
double Tensor::getElement(size_t i1, size_t i2, size_t i3, size_t i4)
{	
	return m_pTensor->getElementAt(i1, i2, i3,i4);
}

void Tensor::getElements(std::vector<double>& data)
{
...
void Tensor::modeMultiply(const std::vector<double>& matrixU, const std::string& type, const size_t numRows, const size_t numColumns, const size_t mode, Tensor& outTensor)
{	
	if(type!="N" && type!="T")
	{
		return;
...
void Tensor::unfold(const size_t mode, std::vector<double>& unfoldedTensor)
{
	size_t e1(0); 
	size_t e2(0); 
	size_t e3(0); 
